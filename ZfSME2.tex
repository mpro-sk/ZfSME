...
%% Tlač pod/sekcie doplnená o zarovnanie textu vľavo (\fL)
  ...
  \nobreak}%
  \fi
%% Tlač pod/sekcie doplnená o možnosť vloženia poznámky pod čiarou aj pri
%% požiadavke na dodržanie riadkového registra
\ifnum\RiadkovyRegister=1
\ifnum\SecNote=1
  \def\printsec#1{{\par\norempenalty-400
     \lineskiplimit=-24pt\skiplines{1}
     \def\SkipB{.55}%
    {\IfL\secfont \noindent \dotocnum
    {\thetocnum\quad}#1\nbpar}%
     \insertmark{#1}%
    \ifnum\prevgraf>1 \def\SkipB{.1}\ifnum\prevgraf>2 \def\SkipB{.66}\fi\fi 
    \nobreak \skiplines{\SkipB} \firstnoindent
  }}% 
  \def\printsecc#1{{\par\norempenalty-200
     \LineSkipOrg=\baselineskip \lineskiplimit=-24pt\skiplines{1}
     \def\SkipB{.66}%
    {\IfL\seccfont \noindent \dotocnum
    {\thetocnum\quad}#1\nbpar%
     \ifnum\prevgraf>1 \def\SkipB{.5}\ifnum\prevgraf>2 \def\SkipB{.3}\fi\fi 
     \nobreak \skiplines{\SkipB} \firstnoindent}
   }}%
\fi\fi
