...
%
% Upravené makrá ZfSME.tex vedia dodržať riadkový register v prípade formátov
% určenými pre tlač, ale pokiaľ nie je nastavený parameter \RiadkovyRegister
% na hodnotu 1, výstup bude zhodný, ako pri použití makier ZfSME.tex.
% Zároveň boli doplnené aj makrá zo súboru ZfSMEscenar.tex.
%%% Len testovacia verzia vo vývoji, ešte nie je upravené všetko potrebné !!!

%%%            Príklad konfigurácie v sparacovávanom súbore .tex          %%%
%% V súbore ZfSME2.tex nechať všetky možnosti zakomentované, nastavovať    %%
...
%\margins/2 a5 (14,22,18,24)mm % formát A5, P/Ľ zrkadlo, 40 riadkov/strana
...
%\input \ZFroot BBQ10      % nastavenia pre Black Berry Q10
%\let\SmallSkip=\smallskip % pre kompatibilutu s pôvodným ZfSME
%\let\MedSkip=\medskip     % pre kompatibilutu s pôvodným ZfSME
%\def\RiadkovyRegister{0}  % po odkomentovaní funguje RR s ZfSME2
%% odkomentovať len jeden z nasledujúcich 2 riadkov:
%\input \ZFroot ZfSME2 % upravené makrá reagujú na parameter pre RR
%\input \ZFroot ZfSME % pôvodné makrá nevedia dodržať riadkový register
%\ifx\RiadkovyRegister\undefined \def\RiadkovyRegister{0} \fi % pre k. s ZfSME
%% Upresnenie umiestnenia veľkých začiatočných písmen (až po načítaní ZfSME)
%% (nie je potrebné, ak sa používa ZfSME2, tam sa to nastaví automaticky)
...
%\declCap Ľ {0:24;0,0} \declCap L {0:24;0,0} %odkomentovať len pre font Charter
...
\ifx\MfT\undefined \let\MfT=\it\fi % formát pre text \Motto aj \Vers
\ifx\MfA\undefined \let\MfA=\sl\fi % fotmát pre autora v \Motto
...
\ifx\RiadkovyRegister\undefined \def\RiadkovyRegister{0}\fi
\ifx\SmallSkip\undefined \let\SmallSkip=\smallskip\fi
\ifx\MedSkip\undefined \let\MedSkip=\medskip\fi
\ifnum\LogoZF<0 \def\RiadkovyRegister{0}\fi    % RR len pre tlačové formáty
\isdefined{ptex }\iftrue\else\newdimen\ptex\fi % ak nie je v konfigurácii 
\ifnum\RiadkovyRegister = 1 % TBN, str. 114
\bigskipamount=12pt 
\medskipamount=6pt 
\raggedbottom
...
\let\MedSkip=\bigskip
\fi
\ifx\NumColumns\undefined \def\NumColumns{1}\fi
\ifnum\NumColumns>1 \input eplain \fi
...
\def\fXi{\leftskip=\iindent plus1fil
         ...
         \parfillskip=0pt plus2fil}
\def\IfL{\ifnum\SecLeft=1\fL\fi{}} % \fL len ak \SecLeft=1

%%% [tbn, str. 113] Vynechání místa se zachovaným řádkováním i na zlomu stran
\def\skiplines#1{\par\hbox{}\nobreak\vskip-\baselineskip 
     \vskip#1\baselineskip}
%%% [tbn, str. 115] Umístí libovolně velký box do řádkové osnovy
...
%% Tlač pod/sekcie doplnená o zarovnanie textu vľavo (\fL)
  ...
  \else % zachovanie riadkového registra
  % + doplnenie poz. pod čiarou pomocou AI Google - viď. poznámky %
  \def\printsec#1{\par \norempenalty-400 \bigskip
    % 1. Otvoríme lokálnu skupinu. Všetky zmeny vnútri (vrátane \def\fnote) 
    % automaticky zaniknú pri uzatváracej zátvorke }.
    {% 
       \def\fnote##1{%
          \fnotemark 1% Vytlačí vizuálnu značku v nadpise cez OPmac
          \gdef\NoteText{##1}% Globálne uložíme text, aby prežil koniec skupiny
       }%
    % Vycentrujeme box do riadkovej osnovy
    \vycentruj\vbox{%
    {\IfL\secfont \noindent \dotocnum
    {\thetocnum\quad}#1\nbpar}%
    \remskip.6\bigskipamount}%
    \insertmark{#1}\firstnoindent\nobreak
    }% 2. Koniec skupiny. \fnote je od tohto momentu opäť pôvodné makro z OPmac!
    %
    % 3. Sme späť v hlavnom prúde. Ak nadpis obsahoval poznámku, vytlačíme ju.
    \ifx\NoteText\undefined \else
       \fnotetext{\NoteText} % Natívne OPmac vypísanie textu pod čiarou
       \global\let\NoteText=\undefined% Vymažeme kontajner
    \fi
  }% 
  \fi
  \def\printsecc#1{%
...
\ifnum \RiadkovyRegister = 1
\def\seccc#1\par{\norempenalty-100 \skiplines{.7} \noindent
{\bf\IfL#1\nbpar}%
\nobreak\vskip.3\baselineskip  \firstnoindent}
\else
...
% Makro \TypoInfo pre použitie v českej aj slovenskej verzii textu
% (pomocou makier \sdef a \mtext z balíka OPmac):
\sdef{mt:typoinfo:sk}%
...
\ifdim\vsize > 120mm \vskip 48pt\noindent \else \bigskip\noindent \fi
\mtext{typoinfo}}
...
\ifnum \RiadkovyRegister = 1
\def\Itemm#1\par{\skiplines{.5}\item #1\nobreak\medskip}
\else
\def\Itemm#1\par{\smallskip\item #1\nobreak\smallskip}
\fi

% Oddeľovač hviezdička medzi odstavcami (2/2025 zmena smallskip->medskip):
\def\Hviezdicka{\nobreak\skiplines{.5}\centerline{*}\medskip}
...
{\ifnum\NumColumns>1 \let\smallskip=\SmallSkip \fi
    \removelastskip\smallskip\noindent
                 \MfT #1\smallskip}\else
{\ifx\NoBr\undefined \let\NoBr=\allowbreak \fi
    \removelastskip\SmallSkip\noindent\nnarrower\fL
                 \MfT #1\par\NoBr\SmallSkip}\fi
                }
...
% Pre zlomkovú čiaru sa používa \exfractionsolidus z balíka exchars.tex
\input exchars
\def\FR#1/#2 {{\ifnum\FRformat=0 
\def\tmp{Charter}\ifx\FontType\tmp
$^{#1}\!/\!_{#2}$\else % pôvodná, typograficky menej vhodná definícia
\def\tmp{Antykwa To\-ru\'nska}\ifx\FontType\tmp
$^{#1}\!/\!_{#2}$\else % pôvodná, typograficky menej vhodná definícia
\def\tmp{Antykwa Pó\l taw\-skiego}\ifx\FontType\tmp
$^{#1}\!/\!_{#2}$\else % pôvodná, typograficky menej vhodná definícia
\typoscale[860/]%
% doplnenie zmeny formátovania pre Palatino, LM a CS-CM
\def\tmp{Palatino}\ifx\FontType\tmp
\hbox{$^{#1}\!$}{\typoscale[1163/]\exfractionsolidus}%
\hbox{$\scriptstyle{#2}\,$}%
\else
\def\tmp{Latin Modern}\ifx\FontType\tmp 
\hbox{$\!^{#1}\!$}{\typoscale[1163/]\exfractionsolidus}%
\hbox{$\!\scriptstyle{#2}\,$}%
\else
\ifx\FontType\CMCS 
...
\hbox{$\!\scriptstyle{#2}\,$}%
\else
\hbox{$^{#1}$}{\typoscale[1163/]\exfractionsolidus}\hbox{$\scriptstyle{#2}$}%
\fi\fi\fi\fi\fi\fi
\else
$\,{#1\over#2}\,$\fi}}
% Prepínač formátu zlomkov
\def\FRswitch{\ifnum\FRformat=0\def\FRformat{1}\else\def\FRformat{0}\fi}

% Polovičná medzera voči \, (t. j. polovica \thinspace)
\def\:{\ifmmode\mkern2mu\fi }
% pre niektoré fonty však môže byť definovaná odlišne:
\def\tmp{Latin Modern} \ifx\FontType\tmp 
\def\:{\ifmmode{}\fi} \else
\ifx\FontType\CMCS 
\def\:{\ifmmode{}\fi} \else
\def\tmp{Palatino} \ifx\FontType\tmp
\def\:{\ifmmode\mkern4mu\fi } 
\fi\fi\fi
...
\else
\sdef{mt:chap:sk}{\hfill\vrule height12pt depth2pt width14pt\relax }
\sdef{mt:chap:cs}{\hfill\vrule height12pt depth2pt width14pt\relax }
\sdef{mt:chap:en}{\hfill\vrule height12pt depth2pt width14pt\relax }
\fi
...
% + doplneneie definíce \ChapTitle a \headchap do makra \chap
\eoldef\chap#1{\ifnonum\else \global\advance\chapnum by\Incrmc \fi
  ...
  \xdef\headchap{\ifnonum\else\thechapnum~\fi} % doplnené do pôvodnej
  \def\ChapTitle{#1}                           % definície \chap
}
% Doplnenie začiatočnej nepárnej strany kapitoly  do makra \printchap
\def\printchap#1{\null\vfill\supereject
  \NewPageIfEven
  \ifnum\RiadkovyRegister = 1
    \vycentruj\vbox{% pre názov kapitoly sa nepredpokladá pozn. pod čiarou
...
\def\LaveZahlavie{\ifx\ChapTitle\null \else \def\MySpace{4pt}\fi
                 {\ifx\ChapTitle\firstmark \it \else \sl \fi
                  \ifx\headchap\undefined \def\headchap{} \fi
                  \headchap\HeadSize\vphantom{\^Č}% 08/2026 doplnenie \headchap
...
\ifnum\RiadkovyRegister=1\vycentruj\fi
...
\ifnum\RiadkovyRegister=1\vycentruj\fi\vbox{
\medskip
\ifnum\ObrPdf = 2       % vloží eps obrázok (pre dvi výstup)
\leftline{#6\InsPice{#5}{0pt}}
\else
...
% Výpočet zlatého rezu pre umiestnenie \vbox-u ľubovoľnej výšky na strane
% výšky \vsize (pomocou AI Google - viď. poznámky):
% 1. Alokácia pomocných registrov (stačí definovať raz na začiatku dokumentu)
\newbox\MojBox
\newdimen\ZbytokVysky
\newdimen\HornaMedzera
% 2. Definícia samotného makra
\long\def\ZlatyBox#1{%
  % Uložíme obsah do pomocného boxu, aby sme zistili jeho výšku
  \setbox\MojBox=\vbox{#1}%
  % Vypočítame voľné miesto na strane: \vsize - výška boxu - výška čiary:
  \ZbytokVysky=\vsize
  \advance\ZbytokVysky by -\ht\MojBox
  \advance\ZbytokVysky by -0.4pt % pre čiaru \hrule s hrúbkou 0.4pt
  % Vynásobíme voľné miesto koeficientom 0.38197 pre hornú medzeru:
  \HornaMedzera=.38197\ZbytokVysky
  % Vygenerujeme samotnú stranu s boxom:
  \vbox to \vsize{%
    \KindleHR                 % Fixný prvok na úplnom začiatku strany
    \vskip\HornaMedzera       % Medzera sa teraz neodstráni
    \box\MojBox               % Vloženie samotného boxu
    \vfil                     % Pružná medzera dopočíta zvyšok strany do \vsize
  }%
  \eject                      % Sadzba hotovej strany
}
...
% Nová definícia pre titulnú stranu s presným umiestnením boxu s titulom
% a autorom do zlatého rezu výšky strany (potrebné makro \ZlatyBox)
...
{\hoffset=\HoffsetTitul \voffset=\VoffsetTitul \vsize=\VsizeTitul
\ZlatyBox{%
{\noindent\fC\titfont\Titul\par}
\AuthorF \Autor\par
}
\ifnum\pageno=-2 \NewPageIfEven \fi
}}
...
\fi
% Výnimky pre písmo Computer Modern vo variante CS-font
\ifx\FontType\CMCS 
\declCap Ľ {0:30;2,-2}
\ifdim\hsize < \MinHsize 
\declCap Č {-1.2:31;0,-2} \else
\declCap Č {1:31;0,-2} \fi
\ifnum\CapNumLines=3
\declCap Ľ {0:41;2,-2,-2}
\declCap Ď {1:40;2,-1,3}
\fi\fi
%% To čo je pre ZfSME potrebné nastaviť ručne v konfiguračnej časti:
% Výnimky pre písmo Palatino: 
\def\tmp{Palatino}
\ifx\FontType\tmp
\declCap J {1:26;2,1}
\ifnum\CapNumLines=3
\declCap J {1:38;2,0,0}
\fi\fi
% Výnimky pre písmo Charter:
\def\tmp{Charter}
\ifx\FontType\tmp
\ifnum\CapNumLines=3 \else
\declCap Ľ {0:24;0,0} \declCap L {0:24;0,0}
\fi\fi
% Výnimky pre písmo Antykwa Torunska:
\def\tmp{Antykwa To\-ru\'nska}
\ifx\FontType\tmp
\declCap J {1:26;2,1}
\ifnum\CapNumLines=3
\declCap J {1:38;2,0,0}
\fi\fi
%% ...až potiaľto sa to spraví automaticky
%
...
%%% Doplnené sú aj makrá zo súboru ZfSMEscenar.tex
...
\let\CB=\bf 
\fi
% Kapitálky nefungujú ani v písme Latin Modern, preto:
\def\tmp{Latin Modern}
\ifx\FontType\tmp
\let\caps=\empty 
\let\CR=\empty 
...
\def\iCB#1:{% označenie ďalšej osoby v texte repliky (formát osoba:)
{\def\tmp{#1}\ifx\Osoba\tmp\CB\else\CR\fi #1}}
...
%%% Doplnené sú aj makrá pre viacstĺpcovú sadzbu bežného textu knihy

% Začiatok viacsĺpcovej tlače na základe parametra \NumColumns:
\def\MultiColStart{\ifnum\NumColumns>1
\begingroup
\parskip=0pt
\normalbottom
\emergencystretch=2em % nastavenie väčších medzier bez podmienky \MinHSize
\ifx\gutter\undefined \gutter=1.5em\fi
%\def\gutterbox{\vbox to \dimen0{\vfil\hbox{\vrule height\dimen0}\vfil}}%
\vskip 5\ptex % zatiaľ neviem prečo je takýto posun potrebný
\doublecolumns
\fi}

% Ukončenie viacsĺpcovej tlače na základe parametra \NumColumns:
\def\MultiColStop{\ifnum\NumColumns>1 \singlecolumn\endgroup\fi}
