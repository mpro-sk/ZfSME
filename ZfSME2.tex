...
\ifx\SecNote\undefinned\def\SecNote{0}\fi
...
%% Tlač pod/sekcie doplnená o zarovnanie textu vľavo (\fL)
  ...
  \else
  \def\printsec#1{\ifnum\SecNote=0
    \par \norempenalty-400 \bigskip
    \vycentruj\vbox{%
    {\IfL\secfont \noindent \dotocnum
    {\thetocnum\quad}#1\nbpar}%
    ...
    \else % umožní pri dodržaní riadkového registra aj poznámku pod čiarou
    {\par\norempenalty-400\lineskiplimit=-24pt\skiplines{1}
     \def\SkipB{.55}%
    {\IfL\secfont \noindent \dotocnum
    {\thetocnum\quad}#1\nbpar}%
    \insertmark{#1}%
    \ifnum\prevgraf>1 \def\SkipB{.15}\ifnum\prevgraf>2 \def\SkipB{.66}\fi\fi 
    \nobreak \skiplines{\SkipB} \firstnoindent
   }\fi
  }% 
  \fi
  \ifnum\RiadkovyRegister=0
  ...
  \else
  \def\printsecc#1{\ifnum\SecNote=0
    ...
    \firstnoindent\nobreak
    \else % umožní pri dodržaní riadkového registra aj poznámku pod čiarou
    {\par\norempenalty-200\lineskiplimit=-24pt\skiplines{.7}
     \def\SkipB{.1}%
    {\IfL\seccfont \noindent \dotocnum
    {\thetocnum\quad}#1\nbpar}%
     \ifnum\prevgraf>1 \def\SkipB{.75}\ifnum\prevgraf>2 \def\SkipB{.6}\fi\fi 
     \nobreak \skiplines{\SkipB} \firstnoindent
   }\fi
  }%
  \fi
